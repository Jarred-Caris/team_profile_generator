...
<body>
  ...
  function renderIntern(intern) {
    let generateIntern = `<div class="col-sm">
    <div class="card" style="width: 18rem;">
  ...
  for (i = 0; i < employees.length; i++) {
    ...
    if (currentEmployee.getRole() === "Manager") {
      htmlTemplate += renderManager(currentEmployee);
    } else if (currentEmployee.getRole() === "Engineer") {
      htmlTemplate += renderEngineer(currentEmployee);
    } else {
      htmlTemplate += renderIntern(currentEmployee);
    }
  }
  htmlTemplate += `</body>
</html>`;
